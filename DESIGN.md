...
1. **データのルートへのリンクのラベル。** 同じ画面にある `/company/*` のようなデータのルートへのリンクのうち、1 つの行き先にだけ使われているラベル（企業名）。行き先が 2 件以上ある一覧のときだけ使う（行が 1 件だと「詳しく見る」も 1 つの行き先にしか使われないため）。全行で同じラベル（「詳細」）はデータではない。
...
| meta.signatureVersion | number | シグネチャの計算方法の版（今は 3）。前回と違えば差分に注意書きを付ける |
