...
「戻る」を使わず起点から再現する理由は、SPA では履歴の戻りが状態を正しく戻さないことが多いためである。遅くなる代わりに、どの画面にも決定的な経路で到達できる。

### 操作対象の列挙

可視の `a[href]`、`button`、`role=button|tab|menuitem|link`、`input[type=submit|button]`、`summary`、`onclick` 付き要素を対象にする。ラベルは aria-label → innerText → value → title → img alt の優先順で決め、同じラベルが複数あれば出現順の番号で区別する。
...
2. 列挙と同じ規則で対象要素を見つけ直し、印を付けてクリックする（4 秒でタイムアウト）
3. `domcontentloaded` → `networkidle`（最大 3 秒）→ 設定の待ち時間（既定 600 ms）の順に待つ
...
| 要素 | 内容 | 正規化 |
| --- | --- | --- |
| path | pathname とクエリ | 連続する数字を `#` に置換（UUID も数字部分が潰れる） |
| structure | 可視の見出し（h1〜h3）、操作対象の role・ラベル・href、フォーム項目の type・name | 数字を `#` に置換して連結 |
...
| 一覧の件数差 | 行ごとにリンクがあると件数で骨格が変わる | 同ラベルの操作対象は 1 つに畳む（将来対応） |
| タブ切替 | URL 不変で内容だけ変わる | 骨格に見出しと操作対象が入るので別ノードになる。意図どおり |
| ランダムな ID を含む href | 数字以外の乱数は潰れない | `denyUrlPatterns` か正規化ルールの追加で対応 |
...
除外文言は日本語と英語の両方を既定に含めるが、アプリ固有の危険な操作（「承認」「出荷」「送金」など）は利用者が設定に足す。README にこの点を明記し、設定ファイルのテンプレートにコメントで例を置く。
...
| フロー | 木の葉ごとの「起点からそこまでの経路」を 1 本のフローとみなし、最後の操作を動詞形にした名前（「送信する」を押す → 送信完了）で一覧にする。開くとキャンバスが横一列のコマ割りに変わり、コマ（番号・キャプチャ・画面名・エラー）の間に操作の矢印が入る。← → キーとクリックでコマを進め、右パネルにそのコマの画面の詳細を出す |
...
比較キーはシグネチャであり、ノード id は実行ごとに振り直されるので、実行間で id を突き合わせてはいけない。
...
| 一覧の件数で骨格が変わる | 同一画面が分裂する | 同ラベル操作の畳み込み（シグネチャ側の改善） |
