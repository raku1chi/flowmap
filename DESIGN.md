...
```mermaid
flowchart TD
  A[起点を撮る] --> P[深さ d の画面の操作を計画<br/>共通の操作・同じ形の 2 件目以降は保留]
  P --> W[ワーカーが並列に試行]
  W --> R[新しいコンテキストで<br/>起点から経路を再現・一致を確認]
  R --> F[操作を1つ実行・落ち着くのを待つ]
  F --> G[撮影・シグネチャ算出]
  G --> M[決まった順番で取り込む<br/>既知なら辺だけ・その場の変化なら元の画面に吸収・未知なら新ノード]
  M --> Q{深さ d を<br/>取り込み終えたか}
  Q -- まだ --> W
  Q -- 終えた --> B[保留した操作を<br/>推定・省略・試行]
  B --> C[その場の変化で現れた操作を<br/>元の画面から続けて試行]
  C --> K{次の深さに<br/>画面があるか・上限は}
  K -- ある --> P
...
試す順番と件数は `maxActionsPerPattern` で整える。同じ形の操作（行き先の正規化ルートが同じリンク、データを消すと同じラベルになるボタン。§5）は 1 画面につき 3 件までに絞り、形ごとに 1 件ずつ順繰りに並べてから `maxActionsPerState` で切る。一覧の同種リンクが先頭を占めて他の操作に予算が回らない事態を避けるためで、ZAP の Maximum children に相当する。

### 共通の操作（同じ画面を何度も辿らない）

ヘッダ・フッターのリンクや開閉は全画面にあるので、画面ごとに押すと試行の大半がそれで埋まり、開閉は画面ごとに「開いた状態」のノードを作る。共通の操作は次のように扱う。

- 操作を「キー」で束ねる。リンクは行き先の正規化ルート（`/items/*` など）、それ以外は役割とラベル（数字とデータを潰す。行ごとの「〇〇を並べて比べる」は 1 つのキー）。データを変えうる送信は対象にしない（検索フォームの送信は対象）。
...
操作の結果が別の画面ではなく、今の画面の一部が変わっただけ（比較パネルへの追加、開閉を開く、並べ替え、その場のページ送り）なら、新しいノードにせず元の画面に吸収する。判定は骨格どうしの比較で、次をすべて満たすもの（`isLocalChange`）。

- ルート（正規化したパス）が同じ
- 開いているダイアログと選ばれているタブが同じ（モーダルとタブの切り替えは別の画面として残す）
- 元の画面の見出しを 1 つも失っていない
- 元の画面の操作と入力欄の 8 割以上が残っている

吸収した操作は、ノードの `localActions` に「この画面の中の操作」として記録する。同じ形の操作（行ごとの「〇〇を並べて比べる」）は 1 件にまとめ、画面にある数と押した数、変化で新しく現れた操作のラベル、変化のあとのスクリーンショット（形ごとに 1 枚）を持つ。押しても骨格が変わらない操作（再表示だけのボタン）も同じ一覧に残す。ビューアはどちらも地図に描かず、右パネルに一覧する（§8）。

変化で新しく現れた操作（比較パネルの「比較ページで開く」、メニューの中のリンク）は、元の画面から続けて押す操作として探索する。途中の操作は辺の `via` に持ち、行き先が別の画面なら「並べる → 比較ページで開く」という元の画面からの 1 本の辺になる。

- 続けて押すのは、その深さの試行（保留した操作を含む）を取り込み終えたあと。段数は 2 まで（「メニュー → サブメニュー → リンク」）、1 画面あたり `maxActionsPerState` 件まで
- 元の画面に同じ形の操作があるもの（パネルの「詳しく見る」が一覧の行のリンクと同じ行き先など）と、この画面自身へのリンクは押さない
- 続けて押した結果がその場の変化だけだった組（「表示順 → 在庫あり優先」）は、他の画面では押さない
- 押す前に、危険な操作の除外（`denyText` など。Jev を有効にしていれば Jev の判定も）を同じように掛ける

一覧の各行の同じボタン（データを消すと同じ形になるもの。§5）は、1 画面につき 1 件目だけを先に押し、1 件目がその場の変化だったら残りの行は押さない（`meta.stats.skippedRepeats`）。

**部品が他の画面にも出るとき。** 比較パネルのように sessionStorage に残る部品は、パネルを開いたあとで移った先の画面にも出る。そのままだと「パネルあり」の画面が全画面ぶん増える。吸収したその場の変化の前後の差（現れた行。元の画面に同じ形があっても、変化で新しく増えた操作を含む）を「部品の行」として覚え、新しい状態が「同じルートの既存の画面 ＋ 部品の行だけ」なら、その画面に合流させる（`variantOf`。元の画面の見出しをすべて残し、操作の 9 割以上が残っていること）。部品として見たことのない行（カートの「購入手続きへ」など）が 1 つでも増えていれば別の画面とする。部品の行の中の企業名は `*` にしてあり、別の画面で企業名が消えずに残っていても当たる。

吸収した状態のシグネチャは元のノードに結び付けるので、同じ状態にもう一度着けばすぐ分かる。ただし「戻る」や変化の無かった操作のあとで次の操作にそのまま進んでよいのは、元の画面の代表と同じシグネチャのときだけで、吸収した状態（開閉を開いたまま）からは続けない。

この扱いは `absorbLocalChanges`（既定 true）で切れる。false にすると、その場の変化も別のノードになる（以前の動き）。

実アプリ（localhost:8787、2026-09-26 11 時の実行）では、画面数上限 40 のうち 20 がその場の変化だった。企業一覧の「〇〇を並べて比べる」（aria-label に企業名）を押すたびに比較パネルに企業名が入り、16 の別画面になっていた。ほかに、トップの「重視する点」を開いた状態、検索結果とランキングでパネルを開いた状態、外部サイト 5 つ、絞り込み違いの一覧 2 つがあった。デモアプリにも同じ仕掛け（一覧の各行の「〇〇を比較に追加」と、sessionStorage に残る比較トレイ）を入れてあり、今のエンジンは 17 画面・試行 103 回・約 1 分 15 秒で、トレイを開いた状態は別の画面にならない。
...
画面ごとに、URL、タイトル、スクリーンショット、本文テキストのハッシュ（volatile の中は除く）、開いているダイアログと開閉、コンソールエラー（`console.error`・`pageerror`）、失敗したリクエスト（接続失敗と 4xx／5xx 応答。画面遷移で打ち切られただけのもの（ERR_ABORTED）は除く）を記録する。既知の画面に再訪した場合もエラーは追記する。開けなかった外部サイトはブラウザのエラーページではなく、行き先の URL で記録する。外部サイトを押さないとき（既定）は、画面ごとに外部リンクのラベルと URL を `externalLinks` に残す。
...
| 外部サイト | 押さない（`followExternalLinks: false`） | 画面ごとに外部リンクの一覧だけ残す。`true` にすると押して撮影だけする（操作は列挙しない） |
...
| structure | 開いているダイアログ、選ばれているタブ、可視の見出し（h1〜h3）、操作対象、フォーム項目の type・name | 数字を `#` に置換。操作対象はリンクなら行き先の正規化ルート（別オリジンはホスト名だけ）、それ以外はラベルで表し、**同じ形のものは 1 つに畳んで並べ替える**（DOM の出現順に依存させない）。今の画面と同じルートへのリンク（ページ送り・絞り込み・ページ内リンク）は入れない。開閉のラベルは見ない。`volatileSelectors` の中の要素は入れない。見出しとラベルからはデータの値（URL のデータ区間の値と、画面の中のデータ）を消し、行ごとの同種のボタンは 1 つの形に畳む。同じ見出しの繰り返しは 1 行にする |
| 外部サイト | ホストと、ID を潰したパス | 撮影のみ、structure は使わない |

開閉のラベルを見ないのは、ラベルに今の値が出るからである（「重視する点: 均等」「表示順: 在庫あり優先」）。値が変わっても同じ画面である。開いたかどうかは中身の操作が増えることで骨格に出るが、開いた状態はその場の変化として元の画面に吸収する（§4）。ダイアログとタブは骨格に名前で入れるので、モーダルとタブ違いは別ノードになる（意図どおり）。
...
URL に出ないデータ（一覧の各行の企業名・商品名）も、見出しとラベルから消す（`pageData`）。一覧の各行に「ＡＩＡＩグループ株式会社を並べて比べる」のようなボタン（aria-label に企業名）があると、企業名ごとに別の形の操作になり、1 画面で 100 件以上の操作を別々に試すうえ、データ（どの企業が一覧に載っているか）で骨格が変わってしまう。データの値は 2 つの手がかりで見つける。

1. **データのルートへのリンクのラベル。** 同じ画面にある `/company/*` のようなデータのルートへのリンクのうち、行き先の半分未満にしか使われていないラベル（企業名。同名の別会社が 2 社あっても名前として扱う）。行き先が 2 件以上ある一覧のときだけ使う（行が 1 件だと「詳しく見る」も 1 つの行き先にしか使われないため）。全行で同じラベル（「詳細」）はデータではない。
2. **共通の前置き・後置きを持つボタンの群。** リンクでない操作で、共通部分（3 文字以上）を持ち、違う部分が 2 文字以上のラベルが 4 つ以上あれば、違う部分をデータとみなして `*を並べて比べる` に畳む。共通部分は、群の大きさが最大の 8 割以上あるもののうち最も長いものを選ぶ（単独の「並べて比べる」ボタンに引きずられて「*比べる」まで縮まないように）。

値を消すときは空白の並び（全角空白を含む）を 1 つの半角空白に揃えてから比べる。リンクのラベルは innerText を詰めて取り、ボタンの aria-label はそのまま取るので、「アクシアル　リテイリング株式会社」のような全角空白を含む企業名がリンクとボタンで一致しなくなるためである。

それでも名前を消せなかった行（リンクの無い行など）は、名前を消せた 2 行以上と同じ前置き・後置きを持ち、合わせて 4 行以上なら同じ形に畳む。消せた行は 2. の群から外れるので、これをしないと消せなかった数行だけが骨格に残り、どの企業が一覧に載っているかで骨格が変わる（8787 で `/companies` が絞り込みごとに 3 つに分かれた原因）。

畳んだ形は、試す順番（`maxActionsPerPattern`）、共通の操作のキー、「この画面の中の操作」のまとめ（§4）でも 1 つの形として扱う。

### クエリで絞り込む一覧

クエリは既定で見ないので `/companies?has=telework` と `/companies` は同じルートになるが、h1（「テレワーク制度のある会社」「収録企業の一覧」）が違うので別の画面になっていた。同じ画面に「同じルートへの、クエリだけ違うリンク」が 2 通り以上あれば、そのルートを「クエリで絞り込む一覧」と学習し（`meta.learnedQueryVariants`）、データ区間を持つ画面と同じく h1 の文言を見ない。ページ送り（`?page=2`）も同じ扱いになる。`queryParams` が ignore のときだけで、`structuralParams`・`dataParams` に挙げたパラメータは数えない。学習したらデータ区間と同じく既存のノードのシグネチャを計算し直す。
...
| 行ごとのボタンのラベルにデータ | 「〇〇を並べて比べる」が企業ごとに別の操作・別の骨格になる | 画面の中のデータとして消し、1 つの形に畳む。1 件目がその場の変化なら残りの行は押さない |
| その場の変化 | 比較パネルへの追加・開閉を開くたびに別の画面になる | 元の画面に吸収し、現れた操作は元の画面から続けて押す（§4） |
| 部品が他の画面にも出る | ストレージに残った比較パネルが移った先の画面にも出て、全画面が「パネルあり」で増える | 部品の行だけが増えた状態は既存の画面に合流させる（§4） |
| 絞り込み違いの一覧 | クエリで絞り込んだ一覧が h1 の違いで別の画面になる | クエリだけ違うリンクから「クエリで絞り込む一覧」を学習し、h1 を見ない |
...
| 画面の詳細（右パネル） | 起点からの経路のパンくず、拡大キャプチャ（変化画面は前回と並べる）、グループ・深さ・見出し・ダイアログ・開いている開閉・シグネチャ、できる操作と行き先（他のグループならグループ名を添え、押さずに推定した辺には「推定」と付ける）、この画面の中の操作、外部リンク、ここへ来る操作、この画面を通るフロー、コンソールエラー、失敗リクエスト。再現が一致しなかった画面は注意を出す |

**自画面が再表示されるだけの操作は描かない。** 画面の中で表示が変わるだけの操作も、全部描くと地図が埋まるので描かない。地図（グループ図・木・フロー）に描くのは別の画面へ移る操作だけで、画面の中で完結する操作は右パネルの「この画面の中の操作」に、同じ形の操作を 1 行にまとめて出す。たとえば「[〇〇を比較に追加] 同じ形の操作 16 件（1 件だけ押して確かめた）・現れた操作: 比較ページで開く・すべて外す」と、押したあとのサムネイル（押すと拡大）を並べる。押しても画面の構成が変わらない操作は、名前だけを 1 行に並べる。カードには ◇ と件数を付ける。

その場の変化で現れた操作から別の画面へ行ける場合は、「[〇〇を比較に追加] → 比較ページで開く」のように途中の操作を前に付けた 1 本の辺として描く。フローの名前とクリック数にも途中の操作を含める。外部サイトは撮影しないので、右パネルに「外部リンク（撮影していません）」として一覧する（押すと新しいタブで開く）。
...
画面名はページのタイトルから作るが、`/company/1234` のような同じテンプレートの画面は、タイトルに企業名や商品名が入っている。これをそのまま使うと、たまたま最初に撮った 1 件の名前で画面全体を呼ぶことになるので、データの部分を「〇〇」に置き換えたテンプレート名（「〇〇の働きやすさデータ」）で表示する。撮影した実例は「例: ＡＩＡＩグループ株式会社 ほか 3 件」としてカードと右パネルに添える。同じ置き換えを、辺のラベル（「[〇〇を外す]」）、その場の変化を経る操作、フロー名、この画面の中の操作（現れた操作を含む）、副題（ダイアログ名・開閉のラベル）にも当てる。失敗した操作と、右パネルの URL・見出しの一覧は、どの要素か分かるよう元のまま出す。表示だけの処理で、シグネチャや実行間の比較には関わらない。

| データとみなす値 | 例 |
| --- | --- |
| URL のデータ区間（`*`）の値と、名前だけ残したクエリの値 | `/companies/サービス業` の「サービス業」、`/search?q=flowmap` の「flowmap」 |
| データを表す見出し | 企業詳細の h1「ＡＩＡＩグループ株式会社」 |
| 実例どうしで違うタイトルの部分（代表のタイトルに上の値が見つからないときだけ） | 商品詳細の「ノートPC」「キーボード」。その実例へのリンクのラベル（一覧の行の商品名）からも消す |
| 探索が行ごとの同種の操作として畳んだラベルの〇〇の部分（`localActions[].pattern`） | 「ノートPCを比較に追加」（形は `*を比較に追加`）の「ノートPC」 |
| 別の画面でデータと分かった値（上の 4 つを集めた辞書） | 比較画面のタイトルに入る企業名（URL は法人番号なので、企業詳細で分かった企業名で消す）、比較トレイの「ノートPCを外す」の商品名 |

- **見出しがデータか**は、実例が 2 つ以上あれば実例どうしで違うかで決める。実例が 1 つなら、データ区間を持つ画面の先頭の見出しをデータとみなす（シグネチャでも h1 をデータとして扱う）。ただし見出しに URL の値が入っていれば（「サービス業の企業一覧」）、URL の値がデータで残りはテンプレートの文言とみなす。タイトルの中にもうデータ（URL の値・辞書の名前）が見つかっていれば、見出し（印刷用ページの「印刷用ページ」）はテンプレートの文言とみなす。
- **辞書の名前はどの画面のタイトルからも消す。** データ区間を持たない画面（比較ページ）のタイトルにも企業名が入るため。辞書に入れるのは 3 文字以上の値（行ごとの操作から取った名前は 2 文字以上）で、短い一般的な語で無関係な文字を消さないようにする。
- **実例**は、探索が同じルートで合流させた別の URL のタイトルと先頭の見出しを `samples` に最大 20 件残す。古い graph.json には無いので、そのときは URL と見出しだけで判定する。実例はページ内のアンカー（`#breakdown`）を除いた URL で数える（ハッシュルーティングの `#/` は残す）。同じ企業のページ内リンクで着いた状態を別の実例にすると、実例どうしで見出しが同じになり「見出しはデータではない」と判定して、企業名が画面名と辺のラベルに残る。探索もアンカーだけ違う URL は `samples`・`mergedUrls` に残さない。
...
- **辺のラベルとフロー名もそろえる。** 行き先の実例のデータ（企業名）を文言に含むリンクは「〇〇」にし、同じ画面から同じ行き先への「〇〇」は 1 本にまとめて本数を添える（一覧の商品名のリンクは「〇〇 ×2 → 商品詳細」になり、全行で同じ「詳細」はそのまま残る）。リンク以外の操作（比較トレイの「〇〇を外す」）とその場の変化を経る操作（「[〇〇を比較に追加] → 比較ページで開く」）は辞書で置き換える。件数やページ番号のような数字は置き換えない。最後の操作がデータのリンクになるフローは、行き先の画面名で「『〇〇の働きやすさデータ』を開く」と呼ぶ。
...
| ◇ n | この画面の中で完結する操作（表示が変わるもの）の数。中身は右パネル |
...
| キー | 型 | 内容 |
| --- | --- | --- |
| meta.schemaVersion | number | graph.json の形の版（今は 3）。無ければ版 1 |
| meta.signatureVersion | number | シグネチャの計算方法の版（今は 4）。前回と違えば差分に注意書きを付ける |
...
| meta.learnedQueryVariants\[\] | string? | 探索中に学習した「クエリで絞り込む一覧」のルート（`/companies`）。h1 を見ずに絞り込み違いを合流させた |
| meta.stats | object? | 試行・再現・「戻る」で復帰・推定した辺・省いた共通操作・再現の不一致の回数、この画面の中の変化として吸収した回数・現れた操作を続けて押した回数・部品を除いて合流させた回数・同じ形の行で押さなかった数、並列数、所要ミリ秒 |
...
| localActions\[\] | LocalAction? | この画面の中で完結する操作（下の表。最大 30 件） |
| externalLinks\[\] | {label, href}? | 押さなかった外部リンク（`followExternalLinks: false` のとき。最大 30 件、URL の機微なクエリ値は伏せる） |
| truncated | string? | 探索を打ち切った理由 |

LocalAction（この画面の中の操作。同じ形の操作をまとめたもの）

| キー | 型 | 内容 |
| --- | --- | --- |
| key | string | 同じ形の操作を束ねるキー（`button\|*を並べて比べる` など） |
| action | ActionDesc | 代表の操作（最初に押したもの） |
| pattern | string? | 行ごとの同種の操作を畳んだ形（`*を並べて比べる`）。ラベルにデータを含む操作だけ |
| count / tried | number | この画面にある同じ形の操作の数と、押した数 |
| changed | boolean | 画面の骨格が変わったか。false なら押しても再表示だけ |
| revealed\[\] | string? | 変化で新しく現れた操作のラベル（最大 8 件）。これらは続けて押して探索する |
| screenshot | string? | 変化のあとのスクリーンショット（`shots/s002-1.png`。形ごとに 1 枚） |
...
| via\[\] | ActionDesc? | 先に押した、その場の変化を起こす操作（元の画面から順に）。変化で現れた action を押すために通る（「並べる → 比較ページで開く」の「並べる」） |
...
| warning | 比較の前提が崩れているときの注意書き（シグネチャの版・Jev の有無・その場の変化の扱い・探索範囲の設定・起点 URL が違う、今回の探索が途中で止まった） |
| unreliable | 消失と追加が前提の違いを含む（シグネチャの版・Jev の有無・その場の変化の扱いの違い、今回の中断）。ゲートで消失を数えない |
...
比較の前提が崩れているとき（シグネチャの版・Jev の有無・その場の変化の扱い・探索範囲の設定・起点 URL が前回と違う、今回の探索が途中で止まった）は、diff に注意書き（`warning`）を付け、ビューアの問題一覧と概要、summary.md に出す。消失と追加が前提の違いを含む場合は `unreliable` を付け、ゲートでは消失を数えない。
...
外部サイト（撮影だけする別オリジン）のエラーは、このアプリの回帰ではないので数えない。外部サイトは消失にも数えない（撮るかどうかは設定次第で、行き先の URL も相手の都合で変わる）。データ差で「変化」は出続けるため、ゲートの条件には入れない。
...
1 操作あたり「経路再現＋操作＋待ち」で、深さ d の画面なら約 (d+1)×0.7 秒（落ち着き待ちが大半）。これを `workers` 本で並列に進め、共通の操作は推定・省略するので、試行の数は画面数 × 画面固有の操作数に近づく。デモアプリ（17 画面）は並列 4 で約 1 分 15 秒。CI では `maxStates` と `maxMinutes` で上限を決めておく。
...
| followExternalLinks | boolean | false | 別オリジンへのリンクを押して撮影するか。false なら撮らずに、画面ごとに外部リンクの一覧だけ残す |
| absorbLocalChanges | boolean | true | 画面の中で完結する変化（比較パネルへの追加・開閉など）を別の画面にせず元の画面に吸収し、現れた操作を続けて押す（§4）。false で以前の動き（変化ごとに別のノード） |
...
| 制限 | 影響 | 回避策 |
| --- | --- | --- |
| ホバー・ドラッグ・キー操作の UI は対象外 | hover で開くメニューの中身が辿れない | click で開くメニューなら、開いたメニューの中の操作を続けて押して辿る（§4）。hover のみなら要素を click 可能にする |
