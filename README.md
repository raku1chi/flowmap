...
`/company/1234` のような同じテンプレートの画面は、タイトルの企業名・商品名などを「〇〇」に置き換えた名前で表示します（「〇〇の働きやすさデータ」）。辺のラベル・フロー名・この画面の中の操作も同じく「〇〇 ×2 → 商品詳細」「[〇〇を外す]」のように置き換えます（失敗した操作はどの要素か分かるよう元のまま）。撮影した実例はカードと右パネルに「例: ＡＩＡＩグループ株式会社 ほか 3 件」と添えます。自動の名前が読みにくいときは、設定で直接指定できます。表示だけの設定なので、探索し直さずに `pnpm render` で反映できます。
